...
Connection::Connection(const ConnectionValue& strConnInfo, bool noDelay, size_t bufferSize)
	: connection(new ClientSocket(strConnInfo.host.c_str(), strConnInfo.port.c_str())), ioStream(new std::iostream(connection->getStreamBuf())),
	buffer(new Buffer(bufferSize)), transaction(NULL)
{
	if (noDelay)
	{
	...
	if (!strConnInfo.auth.empty())
	{
		authenticate(strConnInfo.auth.c_str());
	}
}
